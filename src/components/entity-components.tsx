...
type EntityHeaderProps = {
    ...
    | { onNew: () => void; newButonHref?: never }
    | { newButtonHref: string; onNew?: never } 
    | { onNew?: never; newButtonHref?: never }
)

export const EntityHeader = ({
    title, description, onNew, disabled,
    newButtonLabel, newButtonHref, isCreating
}: EntityHeaderProps) => {
    return (
  ...
  return (
    <div className="flex items-center justify-between gap-x-2 w-full">
     ...
     <div className="flex items-center justify-end space-x-2 py-4">
      <Button disabled={page === 1 || disabled} variant="outline"
       size="sm" onClick={() => onPageChange(Math.max(1, page - 1))}>
        Previous
      </Button>
...
export const EntityItem = ({
 href, title, subtitle, image, actions,
 onRemove, isRemoving, className,
}: EntityItemProps) => {
          ...
          {actions} {onRemove && (
            <DropdownMenu>
             <DropdownMenuTrigger asChild>
              ...
              ><MoreVerticalIcon className="size-5" /></Button>
             </DropdownMenuTrigger> 
             <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
              <DropdownMenuItem onClick={handleRemove}>
               <TrashIcon className="size" /> Delete
              </DropdownMenuItem> 
             </DropdownMenuContent>
            </DropdownMenu>
          )}
