     ...
     .mutation(async ({ ctx, input }) => {
       ...
       return await prisma.$transaction(async (tx) => {
        ...
        await tx.connection.createMany({
          data: edges.map((edge) => ({
            workflowId: id, fromNodeId: edge.source,
            toNodeId: edge.target, fromOutput: edge.sourceHandle || "main",
  ...
  getOne: protectedProcedure
     ...
     .query(async ({ ctx, input }) => {
       ...
       const edges: Edge[] = workflow.connections.map((connection) => ({
        id: connection.id, source: connection.fromNodeId, target: connection.toNodeId,
        sourceHandle: connection.fromOutput, targetHandle: connection.toInput,        
       }));
  ...
  getMany: protectedProcedure
     .input(z.object({
        ...
        pageSize: z.number().min(PAGINATION.MIN_PAGE_SIZE)
         .max(PAGINATION.MAX_PAGE_SIZE).default(PAGINATION.DEFAULT_PAGE_SIZE),
