    ...
    return (
        <>
         ...
         <EntityHeader title="Workflows" description="Create and manage your workflows" 
          onNew={handleCreate} newButtonLabel="New Workflow" disabled={disabled} isCreating={createWorkflow.isPending}
         />
        </>
    );
};
...
export const WorkflowsPagination = () => {
    ...
    return (
        <EntityPagination disabled={workflows.isFetching} onPageChange={
            (page) => setParams({ ...params, page })
        } 
         totalPages={workflows.data.totalPages} page={workflows.data.page}  />
    )
}
...
export const WorkflowItem = ({
 ...
 return (
    <EntityItem href={`/workflows/${data.id}`} title={data.name}
     ...
     </div>} onRemove={handleRemove} isRemoving={removeWorkflow.isPending}
    />
 )
}