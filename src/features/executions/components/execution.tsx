...
import { Card, CardContent, CardTitle,
...
export const ExecutionView = ({ executionId }: {
    executionId: string
}) => {
    ...
    <Card className="shadow-none">
     ...
     <CardContent className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
       <div><p className="text-sm font-medium text-muted-foreground">Workflow:- </p>
       ...
       {duration !== null ? (
        ...
        {execution.error && (
          <div className="mt-6 p-4 bg-red-100 rounded-md space-y-3">
           <div>
           ...
           {execution.errorStack && (
            <Collapsible open={showStackTrace} onOpenChange={setShowStackTrace}>
             <CollapsibleTrigger asChild>
              <Button variant="outline" size="sm" className="text-red-900 hover:bg-red-200">
                {showStackTrace ? "Hide stack trace" : "Show stack trace"}</Button>
             </CollapsibleTrigger>
             <CollapsibleContent>
             <pre className="text-xs font-mono text-red-800 overflow-auto mt-2 p-2 bg-red-50 rounded">
              {execution.errorStack}</pre></CollapsibleContent>
            </Collapsible>
           )}
          </div>
        )}
